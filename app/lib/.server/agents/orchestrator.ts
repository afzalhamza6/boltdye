...
import { AgentRole, type AgentContext, type DataStreamWriter } from './types';
...
/**
 * Agent orchestrator - manages the workflow between different agents
 */
export async function orchestrateAgents(options: OrchestratorOptions): Promise<OrchestratorResult> {
  const { 
    messages, 
    files, 
    env, 
  ...
  logger.debug('=============== AGENT ORCHESTRATION STARTED ===============');
  ...
  try {
    ...
    logger.debug('=============== RUNNING PROMPT ENHANCER AGENT ===============');
    ...
    // Ensure the output is a string by properly resolving any Promise
    const enhancedPrompt = await ensureString(enhancerResult.output);
    
    // Log the enhanced prompt - make sure we're logging the resolved string, not the Promise
    ...
    logger.debug('=============== RUNNING CODE GENERATOR AGENT ===============');
